...
for($m=0;$m<2;$m+=1){ // Цикл для можливого повторення
    ...
    $n_lines = intdiv($value-1,$col);
    $text_out = "n_lines = {$n_lines}";
    // imagettftext($back, $font_size, 0, 300, 220, $black, $font_file, $text_out);
    // imagettftext($back, $font_size, 0, 300, 200, $black, $font_file, $text_out);
    ...
    if ($n_type_left==0){ // Для горизонтальної лінії
        ...
        imagecopyresampled($back , $object1,123+32*$j-$correction, 123-20*$n_lines+40*$i, 0, 0, 32, 32, 96, 96);
    }
    if ($n_type_left==1){ // Для вертикальної лінії
        imagecopyresampled($back ,$object1, 123-20*$n_lines+40*$i, 140-17+32*$j-$correction, 0, 0, 32, 32, 96, 96);
    }        
    if ($n_type_left==2){ // Для головної діагоналі
        imagecopyresampled($back , $object1,123+32*$j-$correction-20*$n_lines+40*$i, 140-17+32*$j-$correction+4*$n_lines-8*$i, 0, 0, 32, 32, 96, 96);
    }
    if ($n_type_left==3){ // Для вторинної діагоналі
        // imagecopyresampled($back , $object1,123+32*($col-$j-1)-$correction, 140-17+32*$j-$correction, 0, 0, 32, 32, 96, 96);
        imagecopyresampled($back , $object1,123+32*(min($col,$value)-$j-1)-$correction-20*$n_lines+40*$i, 140-17+32*$j-$correction-(4*$n_lines-8*$i), 0, 0, 32, 32, 96, 96);
...
for ($k=0; $k<$value; $k+=1) {
    ...
    if ($n_type_right==0){ // Для горизонтальної лінії
        imagecopyresampled($back , $object2,403+32*$j-$correction, 123-20*$n_lines+40*$i, 0, 0, 32, 32, 96, 96);
    }
    if ($n_type_right==1){ // Для вертикальної лінії
        imagecopyresampled($back ,$object2, 403-20*$n_lines+40*$i, 140-17+32*$j-$correction, 0, 0, 32, 32, 96, 96);
    }        
    if ($n_type_right==2){ // Для головної діагоналі
        imagecopyresampled($back , $object2,403+32*$j-$correction-20*$n_lines+40*$i, 140-17+32*$j-$correction+4*$n_lines-8*$i, 0, 0, 32, 32, 96, 96);
    ...
    if ($n_type_right==3){ // Для вторинної діагоналі
        imagecopyresampled($back , $object2,403+32*(min($col,$value)-$j-1)-$correction-20*$n_lines+40*$i, 140-17+32*$j-$correction-(4*$n_lines-8*$i), 0, 0, 32, 32, 96, 96);
    }
}
